...
pub const SYSTEM_CONFIG_PATH: &str = "/Library/Application Support/capshift/config.kdl";

const DEFAULT_CONFIG: &str = r#"// /Library/Application Support/capshift/config.kdl
//
...
pub fn parse_bindings(src: &str) -> Result<BindingMap> {
    ...
    for node in doc.nodes() {
        ...
        if present != 1 {
            tracing::warn!(
                key_name,
                "config: bind requires exactly one of app=, shell=, key="
            );
            continue;
        }
        ...
        let binding = if let Some(app_id) = app {
            ...
            Binding::Action(Action::AppLaunch {
            ...
            Binding::Action(Action::ShellCommand {
...
mod tests {
    ...
    fn parses_app_binding() {
        let bindings =
            parse_bindings(r#"bind "s" app="com.tinyspeck.slackmacgap" label="Slack""#).unwrap();
        assert_eq!(
    ...
    fn parses_shell_binding() {
        let bindings =
            parse_bindings(r#"bind "t" shell="open -a Terminal" label="Terminal""#).unwrap();
        assert_eq!(
