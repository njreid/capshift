...
unsafe extern "C" fn value_available(
    ...
    HID_STATE.with(|cell| {
        ...
        match state
            .chord
            .process_with_modifiers(usage, down, logical_modifiers)
        {
            KeyOutcome::Swallow => {}
            ...
            KeyOutcome::Forward {
                ...
                debug!(
                    source = usage,
                    target, down, suppress_modifiers, "capshift: forwarding remapped key"
                );
                if down {
...
fn raw_modifier_mask(logical: u8) -> u8 {
    let mut raw = 0;
    if logical & MOD_SHIFT != 0 {
        raw |= 0x22;
    }
    if logical & MOD_CONTROL != 0 {
        raw |= 0x11;
    }
    if logical & MOD_OPTION != 0 {
        raw |= 0x44;
    }
    if logical & MOD_COMMAND != 0 {
        raw |= 0x88;
    }
    raw
}
...
pub fn run(cfg_rx: watch::Receiver<BindingMap>) -> Result<()> {
    ...
    let _manager = unsafe {
        ...
        let cf_page = CFNumberCreate(
            kCFAllocatorDefault,
            3,
            &page_num as *const _ as *const c_void,
        );
        let cf_usage = CFNumberCreate(
            kCFAllocatorDefault,
            3,
            &usage_num as *const _ as *const c_void,
        );
        ...
        let usage_page_key = CFStringCreateWithCString(
            kCFAllocatorDefault,
            c"DeviceUsagePage".as_ptr(),
            K_ENCODING_UTF8,
        );
        let usage_key = CFStringCreateWithCString(
            kCFAllocatorDefault,
            c"DeviceUsage".as_ptr(),
            K_ENCODING_UTF8,
        );
