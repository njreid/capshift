    ...
    pub fn process_with_modifiers(&mut self, hid: u8, down: bool, modifiers: u8) -> KeyOutcome {
        ...
        if self.active {
            ...
            if hid == APPLE_DELETE_HID {
                ...
                return KeyOutcome::Forward {
                    target: self.caps_lock_hid,
                    suppress_modifiers: 0,
                };
            }
...
mod tests {
    ...
    fn delete_without_caps_remains_backspace() {
        let mut chord = ChordState::new(CAPS, bindings());
        assert_eq!(
            chord.process(APPLE_DELETE_HID, true),
            KeyOutcome::Passthrough
        );
        assert_eq!(
            chord.process(APPLE_DELETE_HID, false),
            KeyOutcome::Passthrough
        );
    }
